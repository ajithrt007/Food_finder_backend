...
class Command(BaseCommand):
    help = 'Creates fixtures for django from json csv file given'
    ...
    def convert_csv_to_json(self,input_folder, input_file, output_folder, output_file, model_name):
        ...
        with open(input_path, mode='r', encoding='utf-8-sig') as csvfile:
            ...
            for row in reader:
                ...
                for column_name in reader.fieldnames:
                    fields[column_name.lower().replace(" ", "_")] = row[column_name]
                json_obj = {
                    "model": model_name,
                    "pk": pk,
                    "fields": fields
    ...
    def handle(self, *args, **options):
        ...
        self.convert_csv_to_json(data_dir,"City-Code.csv", fixtures_dir, "City.json", "food.City")
        self.stdout.write(self.style.SUCCESS("Fixture generated for City-Code."))
        self.convert_csv_to_json(data_dir,"Country-Code.csv", fixtures_dir, "Country.json", "food.Country")
        self.stdout.write(self.style.SUCCESS("Fixture generated for Country-Code."))
        
        count=self.insert_restaurent_into_db(os.path.join(data_dir,"restaurants.csv"),fixtures_dir)
        self.stdout.write(self.style.SUCCESS(f"{count} rows added to DB. Fixture generated for Restaurants."))
